...
parser.add_argument("--width",nargs='?', type=float, default=1/np.sqrt(np.pi))
parser.add_argument("--height",nargs='?', type=float, default=0.5)
parser.add_argument("--nodes",nargs='?', type=int, default=100)
args = parser.parse_args()


radius_length = args.width
radius_height = args.height
...
x_cords = radius_length*np.cos(theta)
y_cords = radius_height*np.sin(theta)
